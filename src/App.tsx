...
const algorithmOptions: AlgorithmType[] = [
  "BFS",
  "DFS",
  "A*",
  // "Greedy",
...
function App() {
  ...
  const activeNodeId = useMemo(() => {
    ...
    if (mode === "step") {
      ...
      const safeIndex = Math.max(
        0,
        Math.min(stepIndex, result.expansionOrder.length - 1),
      );
    ...
    return (
      selectedNodeId ??
      result.solutionNodeId ??
      result.expansionOrder.at(-1) ??
      null
    );
  ...
  const visibleTree = useMemo(() => {
    ...
    const nodes =
      mode !== "step"
        ...
        : (() => {
            ...
            const revealedOrder =
              maxOrder >= 0 ? Math.min(Math.max(stepIndex, 0), maxOrder) : -1;
            ...
            return result.nodes.filter((node) => {
              ...
              return (
                ...
                (parent?.expandedOrder ?? Number.MAX_SAFE_INTEGER) <=
                  revealedOrder
              );
            });
          })();
    ...
    const linkedNodes = filteredNodes.filter(
      (node) => node.parentId === null || filteredIds.has(node.parentId),
    );
    const linkedIds = new Set(linkedNodes.map((node) => node.id));

    return {
      nodes: linkedNodes,
      solutionPath: result.solutionPathNodeIds.filter(
        (id) => visibleIds.has(id) && linkedIds.has(id),
      ),
    };
  ...
  const handleRun = () => {
    const resolvedHeuristic = requiresHeuristic(algorithm)
      ? heuristic
      : "remaining-items";
    ...
    setStepIndex(0);
    setSelectedNodeId(
      output.expansionOrder[0] ?? output.solutionNodeId ?? null,
    );
    setPlaying(false);
    ...
    setHistoryRows((prev) => {
      const row: CompareRow = {
        ...
        solutionDepth:
          output.metrics.solutionDepth !== null
            ? String(output.metrics.solutionDepth)
            : "Không tìm thấy",
  ...
  const disablePlayButton =
    !result || mode !== "step" || (!playing && stepIndex >= stepMax);
  ...
  const compareColumns = [
    ...
    {
      title: "Độ sâu lời giải",
      dataIndex: "solutionDepth",
      key: "solutionDepth",
    },
          ...
          <div className="hero-toolbar">
            <div>
              ...
              <Paragraph className="hero-subtitle">
                Công cụ giảng dạy trực quan cho bài toán Nông dân - Sói - Dê -
                Bắp cải
              </Paragraph>
            </div>
            ...
            <Row gutter={[16, 16]} align="middle">
              <Col xs={24} md={2}>
                ...
                <Select
                  ...
                  options={algorithmOptions.map((algo) => ({
                    value: algo,
                    label: algo,
                  }))}
                />
                  ...
                  {result ? (
                    <>
                      <Row gutter={[8, 8]} className="compact-metrics-row">
                        ...
                        <Col xs={12} sm={8} xl={4}>
                          <Statistic
                            className="compact-stat"
                            title="Nút đã sinh"
                            value={result.metrics.generated}
                          />
                        </Col>
                        <Col xs={12} sm={8} xl={4}>
                          <Statistic
                            className="compact-stat"
                            title="Biên tối đa"
                            value={result.metrics.maxFrontier}
                          />
                        </Col>
                        <Col xs={12} sm={8} xl={4}>
                          <Statistic
                            className="compact-stat"
                            title="Thời gian (ms)"
                            value={Number(result.metrics.durationMs.toFixed(2))}
                          />
                        </Col>
                        <Col xs={12} sm={8} xl={4}>
                          <Statistic
                            className="compact-stat"
                            title="Độ sâu lời giải"
                            value={result.metrics.solutionDepth ?? "Không có"}
                          />
                        </Col>
                        <Col xs={12} sm={8} xl={4}>
                          <Statistic
                            className="compact-stat"
                            title="Chi phí lời giải"
                            value={result.metrics.solutionCost ?? "Không có"}
                          />
                        </Col>
                      </Row>
                      <Divider />
                      <Flex gap={8} wrap>
                        <Tag
                          color={
                            result.metrics.optimalGuaranteed
                              ? "green"
                              : "orange"
                          }
                        >
                        ...
                        <Tag color={result.solutionNodeId ? "blue" : "red"}>
                          {result.solutionNodeId
                            ...
                            : "Chưa tìm thấy lời giải"}
                        </Tag>
                        {result.heuristic && (
                          <Tag color="gold">
                            Hàm đánh giá: {result.heuristic}
                          </Tag>
                        )}
                      </Flex>
                    </>
                    ...
                    <Alert
                      ...
                      description="Chọn thuật toán và bấm Chạy thuật toán."
                    />
                  )}
                </Card>
              </Space>
            ...
            <Col xs={24}>
              ...
              >
                ...
                <Table
                  ...
                  locale={{
                    emptyText:
                      "Chưa có dữ liệu so sánh. Hãy chạy thêm thuật toán.",
                  }}
                />
              </Card>
            </Col>
